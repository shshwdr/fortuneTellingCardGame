...
namespace Naninovel
{
    ...
    public class StateManager : IStateManager
    {
        ...
        public StateManager (StateConfiguration config, EngineConfiguration engineConfig)
        {
            ...
            GameSlotManager = (ISaveSlotManager<GameStateMap>)Activator.CreateInstance(Type.GetType(config.GameStateHandler), config, config.SaveFolderName);
            GlobalSlotManager = (ISaveSlotManager<GlobalStateMap>)Activator.CreateInstance(Type.GetType(config.GlobalStateHandler), config, config.SaveFolderName);
            SettingsSlotManager = (ISaveSlotManager<SettingsStateMap>)Activator.CreateInstance(Type.GetType(config.SettingsStateHandler), config, config.SaveFolderName);
