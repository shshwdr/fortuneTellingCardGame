...
namespace Naninovel
{
    ...
    public class AudioManager : IStatefulService<SettingsStateMap>, IStatefulService<GameStateMap>, IAudioManager
    {
        ...
        {
            if (!audioLoader.IsLoaded(path)) await AudioLoader.Load(path);
        ...
        {
            var clipResource = await audioLoader.Load(path, this);
            ...
            {
                var introClipResource = await audioLoader.Load(introPath, this);
                token.ThrowIfCanceled();
        ...
        {
            var clipResource = await voiceLoader.Load(path, this);
            token.ThrowIfCanceled();
            if (!clipResource.Valid)
            {
