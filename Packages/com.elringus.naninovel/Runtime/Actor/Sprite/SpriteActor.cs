...
namespace Naninovel
{
        ...
        where TMeta : OrthoActorMetadata
    {
            ...
            Transition? transition = default, AsyncToken token = default)
        {
            ...
            if (!textureResource.Valid) throw new Error($"Failed to change '{Id}' actor appearance: failed to load '{textureResource.Path}' appearance resource.");

            if (AppearanceLoader.GetLocalPath(textureResource) is { } localPath)
                AppearanceLoader.Hold(localPath, this);
            await TransitionalRenderer.TransitionTo(textureResource, tween, transition, token);
        ...
        protected virtual async UniTask<Resource<Texture2D>> LoadDefaultAppearance (AsyncToken token)
        {
            ...
            {
                defaultAppearance = await AppearanceLoader.Load(defaultAppearancePath, this);
                token.ThrowIfCanceled(GameObject);
            }
