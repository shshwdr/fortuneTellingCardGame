...
namespace Naninovel
{
    public class ScriptAssetPostprocessor : AssetPostprocessor
    {
        ...
        private static void VerifyScenarioRoot (string assetPath)
        {
            if (!assetPath.StartsWithFast(cfg.ScenarioRoot))
                Engine.Warn($"Script '{StringUtils.BuildAssetLink(assetPath)}' is not stored at scenario root. " +
                            $"Make sure to store all script assets under '{cfg.ScenarioRoot}' directory. " +
                            $"You can change the root directory in scripts configuration.");
        }
        ...
        {
            var resolver = new ScriptPathResolver { RootUri = cfg.ScenarioRoot };
